...
public interface ParkingControllerInterface {
    ...
    /**
     * This method retrieves parking's history data from the system. In order to avoid sending huge amounts of user data
     * it used pagination, so that history data from a particular page, of a particular size can be retrieved.
     * This method retrieves account history data by the parking's id. The results are ordered by
     * modification time.
     *
     * @param id         ID of the parking which history data will be retrieved.
     ...
     * @return This method returns 200 OK as a response, where in response body a list of user parking's history data is located, is a JSON format.
     * If the list is empty, this method would return 204 NO CONTENT as the response.
     * 500 INTERNAL SERVER ERROR is returned when other unexpected exception occurs.
     * @throws ApplicationBaseException General superclass for all exceptions thrown in this method or handled by
     *                                  exception handling aspects from facade and service layers below.
     */
    @GetMapping(value = "/{id}/history-data", produces = MediaType.APPLICATION_JSON_VALUE)
    @Operation(summary = "Get history data of a parking", description = "The endpoint is used retrieve list of parking history data from given page of given size.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "List of parking's history data returned from given page of given size is not empty."),
            @ApiResponse(responseCode = "204", description = "List of parking's history data returned from given page of given size is empty."),
            @ApiResponse(responseCode = "500", description = "Unknown error occurred while the request was being processed.")
    })
    ResponseEntity<?> getHistoryDataByParkingId(@PathVariable("id") String id,
